...
- [ ] **INGEST-04**: Embeddings and chunk text are stored in PostgreSQL with the `pgvector` extension (`chunks` table, `vector(1536)` column)
- [ ] **INGEST-05**: An HNSW index on cosine distance is created and maintained for efficient approximate nearest-neighbor retrieval
